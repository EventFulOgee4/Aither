...
import { createSession, getMessages, listSessions, sendMessage } from "../../api/chat";
...
export default function Home() {
  ...
  setSessions(s);
  if (selectIfEmpty && !activeSessionId && s.length) {
  ...
  useEffect(() => {
    (async () => {
      try {
        const s = await refreshSessions(true);
        if (s.length) {
  ...
  async function handleSend() {
    ...
    try {
      // optimistic render
      const tempUser = { id: `temp-${Date.now()}`, role: "user", content: text };
    ...
    <div className="home-root">
      ...
      <div className="home-main">
        ...
        <main className="home-content">
          ...
          {/* Messages */}
          {messages.length > 0 && (
            <div style={{ width: "min(900px, 92vw)", margin: "18px auto", textAlign: "left" }}>
              {messages.map((m) => (
                <div
                  key={m.id}
                  style={{
                ...
                >
                  <div
                    style={{
                      ...
                      padding: "10px 12px",
                      borderRadius: 14,
                      background: m.role === "user" ? "rgba(155,143,230,.25)" : "rgba(255,255,255,.08)",
                  ...
                  >
                    ...
                    {m.role === "assistant" && m.emotion && (
                      <div style={{ fontSize: 11, opacity: 0.65, marginTop: 6 }}>
                        emotion: {m.emotion} {m.confidence != null ? `(${m.confidence})` : ""}
                      </div>
                    )}
                  </div>
                </div>
          ...
          <PromptCard value={input} onChange={setInput} onSend={handleSend} disabled={sending} />

          <div className="examples-label">ASK AITHER ONE OF THE EXAMPLES BELOW</div>
          <ExampleCards />
        </main>
      </div>
    </div>
